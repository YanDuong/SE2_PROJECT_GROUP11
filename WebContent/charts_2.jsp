...
try{
  ...
  /* ps = conn.prepareStatement("select name, totalOfInfected, totalOfRecovered, totalOfDeaths, updatedInfected, updatedRecovered, updatedDeaths from countries"); */
	/*  ResultSet rs = ps.executeQuery(); */
	String name;
	/* int totalOfInfected;
	...
	while(rs.next()){
		...
		/* out.println("<div style = 'width: 50%; margin-left: auto, margin-right: auto; margin-top: 200px;'>Failed to connect to database" + e.printStackTrace() + "</div>"  ); */
		e.printStackTrace();
	} finally {
		if(ps != null) {
			try {
...
/* try{
	conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/corona_database", "root", "12345678");
	ps = conn.prepareStatement("select name, totalOfRecovered from countries");
	...
	while(rs.next()){
		...
		map2.put("label", name);
		map2.put("y", rs.getInt("totalOfRecovered"));
