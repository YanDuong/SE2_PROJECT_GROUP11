...
public class CityCrudImp implements CityCrud {
	...
	public boolean updateCity(City c) {
		...
		Connection conn = DatabaseConnection.getConnection();
		String sql = "update cities set name = ?, totalOfInfected = ?, totalOfRecovered = ?, totalOfDeaths = ?, updatedInfected = ?, updatedRecovered = ?, updatedDeaths = ? where city_id = ?;";
		PreparedStatement ps = null;
		try {
			 ps = conn.prepareStatement(sql);
			ps.setString(1, c.getName());
	...
	public int getTotalOfInfected() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(totalOfInfected) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
	...
	public int getTotalOfRecovered() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(totalOfRecovered) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
	...
	public int getTotalOfDeaths() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(totalOfDeaths) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
	...
	public int getUpdatedInfected() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(updatedInfected) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
	...
	public int getUpdatedRecovered() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(updatedRecovered) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
	...
	public int getUpdatedDeaths() {
		...
		PreparedStatement ps = null;
		try {
			ps = conn.prepareStatement("select sum(updatedDeaths) from cities");
			ResultSet rs = ps.executeQuery();
			rs.next();
			sum = rs.getInt(1);
		} catch(SQLException e) {
